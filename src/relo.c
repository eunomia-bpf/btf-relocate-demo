...
static void btfgen_mark_member(struct btfgen_info *info, int type_id, int idx)
{
	...
	printf("marking member %s of type %s\n",
	       btf__str_by_offset(info->marked_btf, m->name_off),
	       btf__str_by_offset(info->marked_btf, t->name_off));
	m->name_off = MARKED;
}
...
btfgen_mark_type(struct btfgen_info *info, unsigned int type_id, bool follow_pointers)
{
	...
	printf("marking type %s\n", btf__str_by_offset(info->marked_btf, cloned_type->name_off));
